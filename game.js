...
function promptPlayer() {
  //Make the player enter their move for their round.
  let playerSelection = prompt(
    `Round ${i + 1} of 5. Enter one of these moves: rock, paper, or scissors.`
  ).toLowerCase();
  while (!choices.includes(playerSelection)) {
    //Keep asking the player for a move until it is valid.
...
function game() {
  ...
  if (playerScore > computerScore) {
    ...
    return `You win the game! The final score was ${playerScore} to ${computerScore}, so you beat the computer by ${
      playerScore - computerScore
    }.`;
  } else if (playerScore < computerScore) {
    //Computer wins the game
    return `You lost... The final score was ${playerScore} to ${computerScore}, so the computer beat you by ${
